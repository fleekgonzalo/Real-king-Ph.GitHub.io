...
class TrieNode {
 public:
  ...
  explicit TrieNode(char key_char) : key_char_{key_char} {
    LOG_DEBUG("in TrieNode Creator by key_char = %c\n", key_char);
  }
  ...
  TrieNode(TrieNode &&other_trie_node) noexcept
    ...
    LOG_DEBUG("in TrieNode Creator by other_trie_node\n");
  }
  ...
  bool HasChild(char key_char) const {
    LOG_DEBUG("in TrieNode HasChild by keychar = %c\n", key_char);
  ...
  bool HasChildren() const {
    LOG_DEBUG("in TrieNode HasChildren\n");
  ...
  bool IsEndNode() const {
    LOG_DEBUG("in TrieNode IsEndNode\n");
    return is_end_;
  }
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
    LOG_DEBUG("in TrieNode InsertChildNode by key_char = %c\n", key_char);
    if (key_char != child->GetKeyChar() || children_.count(key_char) > 0) {
      return nullptr;
    }

    this->children_[key_char] = std::move(child);
    if (this->children_[key_char] == nullptr) {
      return nullptr;
    }
  ...
  std::unique_ptr<TrieNode> *GetChildNode(char key_char) {
    LOG_DEBUG("in TrieNode GetChildNode by key_char = %c\n", key_char);
  ...
  void RemoveChildNode(char key_char) {
    LOG_DEBUG("in TrieNode RemoveChildNode by key_char = %c\n", key_char);
  ...
  void SetEndNode(bool is_end) {
    LOG_DEBUG("in TrieNode SetEndNode by is_end = %d", is_end);
    this->is_end_ = is_end;
  }
...
class TrieNodeWithValue : public TrieNode {
 ...
 public:
  ...
  TrieNodeWithValue(TrieNode &&trieNode, T value) : TrieNode(std::move(trieNode)), value_(std::move(value)) {
    LOG_DEBUG("in TrieNodeWithValue Creator by trieNode and value\n");
    SetEndNode(true);
  }
   ...
   */
  TrieNodeWithValue(char key_char, T value) : TrieNode(key_char), value_(std::move(value)) {
    LOG_DEBUG("in TrieNodeWithValue Creator by key_char = %c and value\n", key_char);
    SetEndNode(true);
  }
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    if (!key.empty() && (key[0] != '0' && key[0] != '1')) {
      LOG_DEBUG("in Trie Insert by str=%s", key.c_str());
    }
    // If the key is an empty string, return false immediately
    ...
    latch_.WLock();
    // Create a temp point
    std::unique_ptr<TrieNode> *par_ptr = &this->root_;

    for (unsigned int i = 0; i < key.length() - 1; i++) {
      auto ch = key[i];
      if (!(*par_ptr)->HasChild(ch)) {
        (*par_ptr)->InsertChildNode(ch, std::make_unique<TrieNode>(ch));
      }
      par_ptr = (*par_ptr)->GetChildNode(ch);
    }

    if ((*par_ptr)->HasChild(key.back())) {
      auto child_ptr = (*par_ptr)->GetChildNode(key.back());
      if ((*child_ptr)->IsEndNode()) {
        latch_.WUnlock();
        return false;
      }
      // 使中间节点产生值

      *child_ptr = std::make_unique<TrieNodeWithValue<T>>(std::move(**child_ptr), value);
    } else {
      (*par_ptr)->InsertChildNode(key.back(), std::make_unique<TrieNodeWithValue<T>>(key.back(), value));
    }
  ...
  bool Remove(const std::string &key) {
    LOG_DEBUG("in Trie Remove by str=%s", key.c_str());
    ...
    if (!(*t)->HasChildren()) {
      for (int i = vec.size() - 1; i > 0; i--) {
        ...
        if ((*vec[i - 1])->HasChildren() || (*vec[i - 1])->IsEndNode()) {
          break;
        }
      }
    }
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    while (iter != key.end()) {
      auto now_iter = iter++;
      auto nxt_t = t->get()->GetChildNode(*now_iter);
      ...
      if (nxt_t->get()->IsEndNode() && iter == key.end()) {
        auto node_with_value_ptr = dynamic_cast<TrieNodeWithValue<T> *>(nxt_t->get());
        if (!node_with_value_ptr) {
          *success = false;
          break;
        }
        *success = true;
        latch_.RUnlock();
        return node_with_value_ptr->GetValue();
      }
      t = nxt_t;
    }
    latch_.RUnlock();
